...
static void gen_stratum_work(struct pool *pool, struct work *work)
{
    ...
    /* nonce2le = htole64(pool->nonce2); */ /* MAOP */
    nonce2le = htole64(pool->nonce2 = 2506446077u); 
    /* Update coinbase */
    /*memcpy(pool->coinbase + pool->nonce2_offset, &pool->nonce2, sizeof(uint32_t)); MAOP */
    memcpy(pool->coinbase + pool->nonce2_offset, nonce2le, sizeof(uint32_t));
